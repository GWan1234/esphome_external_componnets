...
static const uint8_t MS5803_ADDRESS = 0x77;
static const uint8_t MS5803_CMD_ADC_READ = 0x00;
static const uint8_t MS5803_CMD_RESET = 0x1E;
static const uint8_t MS5803_CMD_CONV_D1 = 0x40;
static const uint8_t MS5803_CMD_CONV_D2 = 0x50;
static const uint8_t MS5803_CMD_READ_PROM = 0xA2;

void MS5803Component::setup() {
  if (!this->write_bytes(MS5803_CMD_RESET, nullptr, 0)) {
    this->mark_failed();
    return;
  }
  delay(100);  // NOLINT
  for (uint8_t offset = 0; offset < 6; offset++) {
    if (!this->read_byte_16(MS5803_CMD_READ_PROM + (offset * 2), &this->prom_[offset])) {
      this->mark_failed();
      return;
    }
  }
}
void MS5803Component::dump_config() {
  ESP_LOGCONFIG(TAG, "MS5803:");
  LOG_I2C_DEVICE(this);
  if (this->is_failed()) {
...
float MS5803Component::get_setup_priority() const { return setup_priority::DATA; }
void MS5803Component::update() {
  // request temperature reading
  if (!this->write_bytes(MS5803_CMD_CONV_D2 + 0x08, nullptr, 0)) {
    this->status_set_warning();
    return;
  }

  auto f = std::bind(&MS5803Component::read_temperature_, this);
  this->set_timeout("temperature", 10, f);
}
void MS5803Component::read_temperature_() {
  uint8_t bytes[3];
  if (!this->read_bytes(MS5803_CMD_ADC_READ, bytes, 3)) {
    this->status_set_warning();
    return;
  }
  ...
  if (!this->write_bytes(MS5803_CMD_CONV_D1 + 0x08, nullptr, 0)) {
    this->status_set_warning();
    return;
  }

  auto f = std::bind(&MS5803Component::read_pressure_, this, raw_temperature);
  this->set_timeout("pressure", 10, f);
}
void MS5803Component::read_pressure_(uint32_t raw_temperature) {
  uint8_t bytes[3];
  if (!this->read_bytes(MS5803_CMD_ADC_READ, bytes, 3)) {
    this->status_set_warning();
    return;
...
// https://www.te.com/commerce/DocumentDelivery/DDEController?Action=showdoc&DocId=Data+Sheet%7FMS5803-01BA03%7FB3%7Fpdf%7FEnglish%7FENG_DS_MS5803-01BA03_B3.pdf%7FCAT-BLPS0036
...
void MS5803Component::calculate_values_(uint32_t raw_temperature, uint32_t raw_pressure) {
